...
- Extend the fixture type-check to compile a generated Start feature against real `@tanstack/react-start`, `@tanstack/react-router` and `@tanstack/react-query` under strict mode. This is the only way to catch the server-route and validator typings drifting, and the repo already does this for Express, Fastify, Hono and Nest.
...
- The server-function typings were confirmed from the package's own exports, but the `.validator()` generic behaviour was read from docs. The fixture type-check is what proves the generated code compiles.
- `@tanstack/react-start` requires Node 22.12 in the consuming project. That binds users of the profile, not domain-driver, whose own floor stays at 20.
- Start's plugin must be able to see through the `-server/functions` import chain. If it cannot, the client repository indirection is the thing that breaks, and the fallback is for the hook to call the server function directly.
